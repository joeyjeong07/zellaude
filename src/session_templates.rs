...
/// The exact literal both host scripts match against. The trailing space is
/// load-bearing: without it a user file beginning `// zellaude-generated-notes`
/// reads as ours and gets overwritten or deleted.
pub const MARKER_PREFIX: &str = "// zellaude-generated ";

/// First line of every generated file: `MARKER_PREFIX`, the version, and the
/// template name. The host scripts touch only files whose first line is this
/// shape *and* whose name field matches the filename, so neither a hand-written
/// layout that shares a name nor a user's copy saved under a different name is
/// ever written or pruned.
...
/// The layout file a template owns.
pub fn layout_basename(name: &str) -> String {
    format!("{name}.kdl")
}

/// The `$1` argument to `PRUNE_LAYOUTS_SCRIPT`: every configured template's
/// layout file, one per line.
///
/// Derived from the templates alone, never from whether they compiled. A
/// template that fails to build keeps whatever it generated last time — "could
/// not regenerate X" must never mean "delete X", and in the all-fail case an
/// outcome-derived list would be empty and prune every generated layout at once.
pub fn keep_list(templates: &[SessionTemplate]) -> String {
    templates
        .iter()
        .map(|template| format!("{}\n", layout_basename(&template.name)))
        .collect()
}
...
# `$1` a path, `$2` the basename it is addressed by. True only when the file's
# first line is exactly "// zellaude-generated <version> <name>" with <name>
# equal to the basename minus ".kdl". The space after the tag stops
# "// zellaude-generated-notes: mine" from passing; the name check stops a
# user's copy of a generated layout, saved under another filename, from passing.
is_zellaude_layout() {
    zl_line=$(head -n 1 "$1" 2>/dev/null)
    case "$zl_line" in
        '// zellaude-generated '*) ;;
        *) return 1 ;;
    esac
    zl_tail=${zl_line#'// zellaude-generated '}
    zl_version=${zl_tail%% *}
    zl_name=${zl_tail#* }
    [ "$zl_tail" != "$zl_version" ] && [ "$zl_name" = "${2%.kdl}" ]
}
...
if [ -e "$target" ]; then
    if ! is_zellaude_layout "$target" "$1"; then
...
/// Only `.kdl` files whose marker names them after their own filename are
/// eligible, so a user's renamed copy of a generated layout is never deleted.
pub const PRUNE_LAYOUTS_SCRIPT: &str = r#"
set -eu

# Kept byte-identical to the copy in WRITE_LAYOUT_SCRIPT; see the comment there.
is_zellaude_layout() {
    zl_line=$(head -n 1 "$1" 2>/dev/null)
    case "$zl_line" in
        '// zellaude-generated '*) ;;
